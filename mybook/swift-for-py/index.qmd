...
이 가이드는 Swift의 기초를 안내하며 파이썬과의 비교를 곁들여 설명하므로 기존 파이썬 지식을 그대로 활용할 수 있다.
...
### Swift란?
Swift는 macOS, iOS, watchOS, tvOS 등을 위한 강력하고 직관적인 프로그래밍 언어이다. Apple Inc.와 오픈 소스 커뮤니티가 개발하고 있다.

### 왜 Swift인가?
- **안전성:** 흔한 프로그래밍 오류 부류를 아예 제거하도록 설계되었다.
- **빠른 속도:** C++에 필적하는 성능을 낸다.
- **현대적:** 옵셔널, 제네릭, 클로저 같은 기능 덕분에 표현력 있고 즐거운 프로그래밍이 가능하다.
- **상호 운용성:** Objective-C 코드와 잘 어울려 작동한다.
...
### Playgrounds
Xcode Playgrounds는 Swift 코드를 실험하고 결과를 바로 확인해 볼 수 있는 대화형 환경이다. 파이썬 인터프리터나 Jupyter Notebook을 쓰는 것과 비슷하다.

### 기본 구문
- 문장은 일반적으로 줄 바꿈으로 끝난다. 세미콜론 (`;`)은 한 줄에 여러 문장을 작성하지 않는 한 선택 사항이다.
- 코드 블록은 중괄호 `{}`로 묶는다.
- 주석:
    - 한 줄 주석: `// 이것은 주석입니다`
...
### 선언
- **상수**는 `let`으로 선언한다. 할당 후에는 값을 변경할 수 없다. (한 번 할당된 파이썬 변수를 생각하라).
- **변수**는 `var`로 선언한다. 값을 변경할 수 있다.
...
### 타입 추론
Swift는 종종 초기 값으로부터 변수나 상수의 타입을 추론할 수 있다.
...
### 타입 안전성
Swift는 타입 안전 언어이다. 컴파일러가 컴파일 시간에 타입을 확인하고 불일치가 있으면 오류로 표시한다는 뜻이다. 명시적 변환 없이는 한 타입의 값을 다른 타입의 변수에 할당할 수 없다.
...
### 이름 규칙
- **카멜 케이스:** Swift는 변수, 상수, 함수, 메서드 이름에 카멜 케이스를 사용한다 (예: `myVariableName`, `calculateArea`).
- **어퍼 카멜 케이스 (파스칼 케이스):** 클래스, 구조체, 열거형, 프로토콜과 같은 타입 이름에 사용한다 (예: `MyClass`, `String`, `Int`).

이는 변수와 함수에 `snake_case`를, 클래스에 `CapWords`(파스칼 케이스)를 권장하는 파이썬의 PEP 8과 유사하다.
...
#### Dictionary (딕셔너리)
순서 없는 키-값 쌍의 컬렉션이다. 키는 같은 타입이어야 하고 값도 같은 타입이어야 한다.
...
```
- 세트 연산 (합집합, 교집합 등)은 두 언어 모두에서 사용 가능하다.

### 튜플
고정된 크기로, 서로 다른 타입의 값을 순서대로 담을 수 있는 컬렉션이다.
...
```
Swift의 튜플은 이름 있는 요소를 허용할 수 있는 등 더 강력하다.

### 옵셔널
Swift는 값의 부재를 다루기 위해 "옵셔널"이라는 개념을 도입했다. 옵셔널 변수는 값을 가질 수도 있고 `nil`(파이썬의 `None`과 유사)일 수도 있다. 누락된 값의 가능성을 명시적으로 드러내므로 안전한 코드를 쓰는 데 큰 도움이 되는 핵심 기능이다.
...
```
옵셔널은 Swift를 제대로 쓰려면 꼭 이해해야 하는 개념이다.
...
!true // false
```
**나눗셈 참고:** Swift에서 `Int / Int`는 `Int`를 결과로 낸다 (소수점 이하 버림). 부동 소수점 나눗셈을 위해서는 피연산자 중 적어도 하나가 부동 소수점 타입이어야 한다.

### Nil-병합 연산자 (`??`)
옵셔널이 `nil`일 경우 기본값을 대신 사용한다.
...
```
구문: `a ?? b`는 `a != nil ? a! : b`와 동일하다.

### 범위 연산자
- **닫힌 범위 연산자 (`a...b`):** `a`부터 `b`까지의 값을 포함한다. `1...5`는 1, 2, 3, 4, 5를 나타낸다.
- **반-열린 범위 연산자 (`a..<b`):** `a`부터 `b` 미만까지의 값을 포함한다. `1..<5`는 1, 2, 3, 4를 나타낸다.
- **단방향 범위:** `a...`, `...a`, `..<a`.
...
#### `if`, `else if`, `else`
구문은 유사하지만 Swift는 조건이 한 문장이라도 중괄호 `{}`를 요구한다. 조건 주위의 괄호는 선택 사항이고 명확성을 위해 붙이는 경우도 많다.
...
#### `switch`
Swift의 `switch` 문은 파이썬에 직접적인 `switch`가 없는 것(파이썬 3.10+에는 `match-case`가 있음)에 비해 매우 강력하고 유연한다.
- Swift의 `switch` 문은 모든 가능한 값을 다루거나 `default` 케이스를 포함해야 한다 (철저해야 함).
- 기본적으로 "fall through"하지 않는다 (일반적으로 `break` 필요 없음). C 스타일의 fall-through를 위해서는 `fallthrough` 키워드를 사용할 수 있다.
- 범위, 튜플, 값 바인딩 같은 다양한 패턴을 일치시킬 수 있다.
...
#### `repeat-while` 반복문
다른 언어의 `do-while` 반복문과 유사하다. 조건이 확인되기 전에 반복문 본문이 적어도 한 번 실행된다.
...
### 제어 전달 구문
- **`continue`**: 반복문의 현재 반복을 건너뛰고 다음 반복으로 진행한다. (파이썬과 동일)
- **`break`**: 반복문이나 `switch` 문을 즉시 종료한다. (파이썬과 동일)
- **`fallthrough`**: `switch` 문에서 실행이 다음 case 블록으로 계속되도록 한다 (C 스타일 동작).
- **`return`**: 함수를 종료하고 선택적으로 반환 값을 제공한다. (파이썬과 동일)
- **`throw`**: 오류 처리에 사용된다 (나중에 다룸).
...
print(message)
```
- Swift 함수는 `func` 키워드를 사용한다.
- 매개변수 타입과 반환 타입은 **반드시** 지정해야 한다.
- `->`는 반환 타입을 나타낸다. 함수가 값을 반환하지 않으면 `-> Void`를 사용하거나 생략할 수 있다.
...
### 인자 레이블과 매개변수 이름
기본적으로 Swift는 함수를 호출할 때 매개변수 이름을 인자 레이블로 사용한다.
...
**다른 인자 레이블 지정:**
내부 매개변수 이름과 다른 외부 인자 레이블을 제공할 수 있다. 인자 레이블이 없도록 하려면 `_`를 사용한다.
...
```
이는 레이블이 항상 매개변수 이름인 파이썬의 키워드 인자와 다르다.
...
함수 매개변수는 기본적으로 상수이다. 함수 안에서 매개변수 값을 고친 결과가 호출이 끝난 뒤에도 유지되길 원한다면 그 매개변수를 in-out 매개변수로 정의한다.
in-out 매개변수에 인수를 넘길 때는 변수 이름 바로 앞에 앰퍼샌드(`&`)를 붙이다.
...
### 일급 시민으로서의 함수
파이썬과 마찬가지로 Swift의 함수는 일급 타입이다. 변수/상수에 할당하거나, 다른 함수에 인수로 전달하거나, 함수에서 반환할 수 있다.
...
### 클로저
클로저는 코드 안에서 전달되고 쓰일 수 있는 자체 포함된 기능 블록이다. 파이썬의 `lambda`와 유사하지만 훨씬 강력해서 Ruby의 블록이나 JavaScript의 익명 함수에 가깝다.
...
**Swift 클로저:**
클로저는 정의된 컨텍스트의 모든 상수와 변수에 대한 참조를 캡처하고 저장할 수 있다 (이를 "클로징 오버"라고 함).
...
**후행 클로저:**
클로저 표현식이 함수의 마지막 인수인 경우, 함수 호출 괄호 뒤에 *후행 클로저*로 작성할 수 있다. 클로저가 *유일한* 인수인 경우 괄호를 완전히 생략할 수 있다.
...
클로저는 Swift 곳곳에서 쓰이다. 특히 콜백, 비동기 작업, 함수형 프로그래밍 패턴에서 유용하다.

## 8. 객체 지향 프로그래밍 {.unnumbered}

### 클래스와 구조체 (`struct`s)
클래스와 구조체 모두 프로퍼티와 메서드를 정의할 수 있다.

**주요 차이점:**
- **클래스는 참조 타입이다:** 클래스 인스턴스를 할당하거나 함수에 넘길 때는 기존 인스턴스를 가리키는 참조를 다룬다. 한 참조에서 바꾼 내용은 다른 참조에서도 그대로 보이다. (파이썬에서 대부분의 객체가 작동하는 방식과 유사하다).
- **구조체는 값 타입이다:** 구조체 인스턴스를 할당하거나 함수에 넘길 때는 인스턴스가 복사된다. 어느 한 복사본을 바꿔도 다른 복사본에는 영향이 없다. (파이썬의 `int`, `str` 같은 기본 타입, 또는 리스트를 `new_list = old_list[:]`처럼 명시적으로 복사할 때와 유사하다).

- **상속:** 클래스는 상속을 지원하지만 구조체는 지원하지 않는다.
- **소멸자 (`deinit`):** 클래스는 소멸자를 가질 수 있지만 구조체는 가질 수 없다.
- **참조 계산:** 참조 계산은 클래스의 인스턴스에만 적용된다.
...
```
구조체의 인스턴스가 `let` 상수에 할당되면, 해당 프로퍼티가 변수 프로퍼티일지라도 변경할 수 없다. 이는 구조체가 값 타입이기 때문이다. 클래스(참조 타입)의 경우, `let` 상수 인스턴스의 변수 프로퍼티를 변경할 수 있다.

### 프로퍼티

프로퍼티는 특정 클래스, 구조체 또는 열거형과 값을 연관시킨다.

#### 저장 프로퍼티
인스턴스의 일부로 상수(`let`) 또는 변수(`var`) 값을 저장한다.
...
**지연 저장 프로퍼티:**
처음 사용될 때까지 초기 값이 계산되지 않는 프로퍼티이다. `lazy var`로 선언한다. 반드시 `var`여야 한다. 초기 값이 계산 비용이 많이 들거나 초기화 시 사용할 수 없는 외부 요인에 의존하는 프로퍼티에 유용하다.
...
#### 계산 프로퍼티
실제로 값을 저장하지 않는다. 대신 getter와 선택적 setter를 두어 다른 프로퍼티와 값을 간접적으로 읽고 설정한다.
...
프로퍼티 값의 변경에 응답한다. 새 값이 현재 값과 동일하더라도 프로퍼티 값이 설정될 때마다 호출된다.
- `willSet`은 값이 저장되기 직전에 호출된다. 새 값은 상수(기본 이름 `newValue`)로 전달된다.
- `didSet`은 새 값이 저장된 직후에 호출된다. 이전 값은 상수(기본 이름 `oldValue`)로 전달된다.
...
#### 인스턴스 메서드
클래스, 구조체 또는 열거형의 인스턴스에서 호출되는 메서드이다. (`self`는 현재 인스턴스를 참조한다).
...
구조체와 열거형은 값 타입이다. 기본적으로 값 타입의 프로퍼티는 해당 인스턴스 메서드 내에서 수정할 수 없다. 수정을 허용하려면 메서드 앞에 `mutating` 키워드를 붙이다.
...
#### 타입 메서드 (정적 메서드)
인스턴스가 아닌 타입 자체에서 호출된다. `static` 키워드를 사용한다. 클래스의 경우 `class`를 사용하여 하위 클래스가 슈퍼클래스의 구현을 재정의하도록 허용할 수 있다.
...
### 초기화 (생성자)
초기화 구문 (`init`)은 새 인스턴스를 생성하기 위한 특별한 메서드이다. 모든 저장 프로퍼티가 초기 값을 갖도록 하는 것이 주된 역할이다.
...
**기본 초기화 구문:**
- 구조체가 사용자 정의 초기화 구문을 제공하지 않으면 자동으로 *멤버별 초기화 구문*을 얻는다 (모든 프로퍼티에 기본값이 있거나 옵셔널인 경우 기본 `init()`도 얻음).
  `struct Size { var width = 0.0, height = 0.0 }`
  `let s = Size(width: 10, height: 20)` 또는 `let s = Size()`
- 클래스가 모든 프로퍼티에 기본값을 제공하고 사용자 정의 초기화 구문이 없으면 기본 `init()`을 얻는다.

**지정 초기화 구문과 편의 초기화 구문 (클래스용):**
- **지정 초기화 구문**은 주 초기화 구문이다. 해당 클래스에서 도입된 모든 프로퍼티를 완전히 초기화하고 적절한 슈퍼클래스 초기화 구문을 호출한다. 클래스는 적어도 하나를 가져야 한다.
- **편의 초기화 구문**은 보조적인 지원 초기화 구문이다. 동일한 클래스의 다른 초기화 구문을 호출해야 한다 (결국 지정 초기화 구문을 호출).
  `convenience` 키워드로 표시된다.
...
실패할 수 있는 초기화 구문, 즉 `nil`을 반환할 수 있는 초기화 구문이다.
...
### 소멸화 (소멸자)
소멸자 (`deinit`)는 클래스 인스턴스가 할당 해제되기 직전에 호출된다. 클래스에만 해당된다.
...
### 상속
클래스는 다른 클래스(슈퍼클래스)로부터 메서드, 프로퍼티 및 기타 특성을 상속받을 수 있다. 상속받는 클래스는 하위 클래스이다.
...
// class MyCar: Car {} // 오류
```
- 슈퍼클래스 메서드, 프로퍼티 또는 서브스크립트를 재정의하려면 `override` 키워드를 사용한다.
- `super.`를 사용하여 슈퍼클래스 멤버에 접근한다 (예: `super.init()`, `super.someMethod()`).

### 프로토콜 (인터페이스)
프로토콜은 특정 작업이나 기능에 필요한 메서드, 프로퍼티 등 요구 사항의 청사진을 정의한다. 클래스, 구조체, 열거형이 이 프로토콜을 *준수*할 수 있다. Java/C#의 인터페이스나 파이썬의 추상 기본 클래스(ABC)와 유사하다.
...
```
프로토콜은 다형성과 코드 재사용(프로토콜 지향 프로그래밍)을 가능하게 하는 Swift 디자인의 기본이다.

### 익스텐션
기존 클래스, 구조체, 열거형 또는 프로토콜 타입에 새로운 기능을 추가한다. 여기에는 계산 프로퍼티, 메서드, 초기화 구문, 서브스크립트 추가 및 프로토콜 준수가 포함된다.
이는 파이썬의 클래스 몽키 패칭 기능과 유사하지만 더 구조적이고 타입 안전하다.
...
Swift는 복구 가능한 오류를 던지고 잡고 전파하는 기능을 언어 차원에서 기본으로 갖추고 있다. 이는 값의 부재를 다루는 옵셔널과는 다른 문제이다. 오류 처리는 작업이 낼 수 있는 예기치 못한 결과를 다룬다.

파이썬은 예외 처리를 위해 `try-except-finally` 블록을 사용한다.

### 오류 표현 및 발생
Swift에서 오류는 `Error` 프로토콜(빈 프로토콜)을 준수하는 타입의 값으로 표현된다. 열거형은 관련된 오류 조건 그룹을 모델링하는 데 종종 적합하다.
...
### 오류 처리
함수가 오류를 발생시키면 프로그램 흐름이 변경된다.

#### `throws`를 사용하여 오류 전파
함수는 선언에 `throws` 키워드를 포함하여 오류를 발생시킬 수 있음을 나타낸다.
`throws` 함수를 호출하는 경우 오류를 처리하거나(`do-catch`, `try?` 또는 `try!` 사용) 현재 함수도 `throws`로 표시하여 오류를 전파해야 한다.
...
`do-catch` 문은 코드 블록을 실행한다. `do` 절 내에서 오류가 발생하면 `catch` 절과 일치시켜 어떤 절이 오류를 처리할 수 있는지 결정한다.
...
#### 오류를 옵셔널 값으로 변환 (`try?`)
`try?`를 사용하여 오류를 옵셔널 값으로 변환하여 처리한다. `try?` 표현식을 평가하는 동안 오류가 발생하면 표현식의 값은 `nil`이 된다.
...
#### 오류 전파 비활성화 (`try!`)
던지는 함수나 메서드가 런타임에 실제로 오류를 발생시키지 않을 것이라고 확신할 때 `try!`를 사용한다. 오류가 *발생하면* 런타임 오류(크래시)가 발생한다. 극도의 주의를 기울여 사용하라.
...
#### 정리 작업 지정 (`defer`)
`defer` 문은 실행이 현재 범위를 벗어나기 직전에 코드 블록을 실행한다 (예: 함수 반환, 오류 발생, 반복문 중단). 리소스 정리에 유용하다. `defer` 문은 나타난 순서의 역순으로 실행된다.
...
## 10. 메모리 관리 {.unnumbered}

Swift는 클래스 인스턴스의 메모리를 관리하기 위해 자동 참조 계산(ARC)을 사용한다. ARC는 해당 인스턴스가 더 이상 필요하지 않을 때 클래스 인스턴스에서 사용한 메모리를 자동으로 해제한다.
- 일반적으로 메모리 관리를 직접 신경 쓸 필요는 없다.
- ARC는 각 클래스 인스턴스를 현재 참조하고 있는 프로퍼티, 상수 및 변수가 몇 개인지 추적하여 작동한다. 해당 인스턴스에 대한 활성 참조가 하나 이상 존재하는 한 인스턴스를 할당 해제하지 않는다.
- **강한 참조 순환:** ARC는 때때로 두 클래스 인스턴스가 서로 강한 참조를 유지하여 절대 할당 해제되지 않는 상황(메모리 누수)을 유발할 수 있다. Swift는 이러한 순환을 해결하기 위해 `weak` 및 `unowned` 참조를 제공한다.
    - **`weak` 참조:** 약한 참조는 참조하는 인스턴스에 대한 강한 유지를 하지 않으므로 ARC가 참조된 인스턴스를 할당 해제하는 것을 막지 않는다. 인스턴스가 할당 해제되면 참조는 `nil`이 된다. 따라서 약한 참조는 옵셔널 타입이어야 한다.
    - **`unowned` 참조:** 약한 참조와 마찬가지로 미소유 참조는 강한 유지를 하지 않는다. 그러나 미소유 참조는 다른 인스턴스가 동일한 수명 또는 더 긴 수명을 가질 때 사용된다. 항상 값을 가질 것으로 가정된다. 인스턴스가 할당 해제된 후 미소유 참조에 접근하면 런타임 오류가 발생한다.

**파이썬 (가비지 컬렉션):**
파이썬은 참조 계산과 순환 가비지 컬렉터의 조합을 사용하여 메모리를 관리한다. 일반적으로 메모리를 수동으로 관리하지 않는다.
...
```
위의 순환을 끊으려면 `ApartmentARC`의 `tenant`를 `weak var tenant: PersonARC?`로 선언해야 한다.

## 11. 고급 주제로 나아가기 {.unnumbered}

- **Apple의 Swift 문서:** 공식 "The Swift Programming Language" 책은 훌륭하고 포괄적인 자료이다.
...
제네릭은 코드를 유연하고 재사용하기 쉽게 만들어 주는 Swift의 강력한 기능 중 하나이다. 파이썬은 동적 타입 언어라 기본적으로 어떤 함수든 여러 타입을 받는다. Swift는 정적 타입 언어이므로, 제네릭으로 타입 안전성을 지키면서 재사용성을 높여야 한다.

### 제네릭 함수

파이썬에서는 타입 힌팅 없이 함수를 작성하면 어떤 타입이든 받을 수 있다.
...
Swift에서 특정 타입에 종속되지 않는 함수를 만들기 위해서는 `<T>`와 같은 타입 매개변수를 사용한다.
...
클래스, 구조체, 열거형도 제네릭으로 정의할 수 있다. 예를 들어, 요소의 타입에 관계없이 동작하는 스택 자료구조를 만들 수 있다.
...
제네릭 매개변수가 특정 클래스를 상속하거나 특정 프로토콜을 준수하도록 요구할 수 있다. 이는 파이썬에서 `issubclass`나 특정 속성의 존재를 기대하는 것(Duck Typing)을 정적이고 안전하게 구현하는 방법이다.
...
```
`Equatable`이라는 프로토콜 제약을 주면, 비교 연산자(`==`)를 안전하게 사용할 수 있다.
...
동시성은 파이썬과 Swift가 서로 다른 길을 간다. 파이썬에서는 GIL (Global Interpreter Lock) 때문에 진정한 멀티스레딩보다 `asyncio` 모듈을 이용한 비동기 프로그래밍 방식(event loop 기반)이나 멀티프로세싱을 주로 쓴다. 반면 Swift는 `async/await` 문법을 지원하며 스레드를 기본으로 삼는 동시성 제어가 가능하다.

### Async/Await 기본 개념

Swift의 `async/await`은 파이썬과 유사하다. 비동기로 작동하는 함수는 `async`로 표시하고 그 결과가 반환될 때까지 기다릴 때는 `await`를 사용한다.
...
- Swift의 `func ... async`는 함수 정의에 붙이며 파이썬의 `async def`와 비슷하다.
- 비동기 작업은 `Task {}` 단위로 묶어 실행할 수 있다.

### 여러 작업 동시에 실행하기

두 개 이상의 비동기 작업을 동시에 수행할 때 파이썬은 `asyncio.gather`를 사용하지만 Swift는 `async let`이나 `TaskGroup`을 제공한다.
...
파이썬에서는 데이터 충돌을 피하려고 `Lock()` 객체 같은 동기화 기본 요소를 쓰지만 Swift 5.5부터는 **Actor**로 여러 스레드에서 동시에 접근하는 상태를 안전하게 격리(Isolate)한다. Actor는 클래스와 비슷하지만 한 번에 하나의 태스크만 내부 상태에 접근할 수 있다.
...
파이썬 개발자들은 주로 `requests` 라이브러리와 `json` 모듈을 사용해 빠르게 API 통신과 파싱을 처리한다. 반면 Swift에서는 기본 기능인 `URLSession`과 `Codable` 프로토콜을 사용해 네트워크 요청과 JSON 인코딩/디코딩 작업을 직관적이면서도 안전하게 처리한다.

### Codable과 JSON

JSON 데이터를 파이썬의 딕셔너리로 다루는 대신, Swift는 타입을 안전하게 변환할 수 있는 `Codable`이라는 프로토콜을 사용한다.
...
파이썬의 동적 딕셔너리(`data["name"]`)와 달리 Swift에서는 미리 정의된 구조체(`User`) 속성에 접근한다. 이는 오타로 인한 오류를 컴파일 단계에서 방지한다.

### URLSession으로 HTTP 요청 가로채기

파이썬의 `requests.get()`에 해당하는 일은 Swift에서 기본 `URLSession`으로 처리할 수 있다. 최근에는 `async/await`가 도입되어 네트워크 요청 코드가 한결 깔끔해졌다.
...
Swift의 `URLSession`은 `requests`만큼 코드가 짧지는 않지만 강타입 시스템 덕분에 안전하고 기본적으로 비동기 환경과 자연스럽게 맞물린다.


***


## 15. 프로퍼티 래퍼 {.unnumbered}

프로퍼티 래퍼는 겉보기에 파이썬의 데코레이터(Decorators)와 매우 비슷하지만 용도는 조금 다른 강력한 기능이다. 값을 저장하고 계산하는 로직을 값 정의 코드에서 분리해 준다.

### 파이썬에서의 데코레이터 패턴과 비교

파이썬에서는 주로 함수의 실행 전후를 감시하거나 상태를 변경할 때 데코레이터를 사용한다 (`@property` 또한 메서드를 프로퍼티처럼 접근 가능하게 한다).
...
Swift에서는 변수가 값을 가지기 직전과 직후의 동작을 감싸서 추상화할 수 있도록 프로퍼티 래퍼를 도입했다. 다음과 같이 변수 이름 위에 파이썬 데코레이터처럼 `@`를 사용한다.
...
프로퍼티 래퍼는 로컬 변수는 물론 SwiftUI 같은 프레임워크의 `@State`, `@Binding`, `@Environment`처럼 상태 관리에도 널리 쓰이며 코드의 보일러플레이트를 덜어 준다.


***


## 16. 불투명 타입과 Any {.unnumbered}

파이썬에서는 변수 선언 시 어떤 타입의 값인지 알 필요가 없기 때문에 오리 타이핑(Duck Typing)을 자유롭게 쓸 수 있다. Swift에서는 타입 안전성을 지키며 다형성을 제공하기 위해 `some`과 `any`라는 키워드를 사용한다.

### Opaque Types (`some`)

반환 타입의 구체적인 구현 세부사항을 감추면서 외부에는 특정 프로토콜에 정의된 기능만 드러내는 방법이다. 파이썬의 타입 힌트 (예: `-> Iterator[int]`)와 유사한 아이디어이다.
...
SwiftUI에서 모든 뷰가 `some View`를 반환하는 것도 같은 원리이다. 컴파일러가 실제 타입을 알 수 있어 반환되는 UI 구조를 최적화하기 좋다.

### Existential Types (`any`)

`some`은 컴파일러가 반환되는 실제 타입을 알고 있어야 하지만 런타임에 여러 종류의 프로토콜 구현체를 다루려면 `any`를 사용한다. 동적 다형성이 필요할 때 사용되는 방식이다.
...
파이썬이라면 단순히 리스트에 담긴 객체의 메서드를 호출하겠지만 `any`는 런타임에 타입 박싱 비용을 갖게 된다는 점을 Swift는 명확히 한다 (성능 관점에서 `some` 사용이 권장됨).
...
결과 빌더는 복잡한 객체나 선언 구문을 간결하게 만드는 Swift만의 기능이다. 대표적으로 SwiftUI가 UI 계층을 그리는 방식이 이 기술로 구현되어 있다.

### 파이썬에서의 DSL 구현

파이썬에서는 주로 메서드 체이닝이나 클래스를 확장해 HTML과 같은 DOM 트리 빌더를 만들지만 블록 수준에서의 자연스러운 계층 표현은 한계가 있다.
...
Swift에서는 이를 위해 파라미터 영역 안의 조건문이나 반복문을 포함한 선언문들을 하나의 결과 구조체로 모아 주는 기능을 갖추고 있다.
...
`@resultBuilder`는 리스트, 텍스트 뿐만 아니라 UI, 정규표현식(Regex Builder) 등 다양한 분야에서 가독성을 획기적으로 향상시킨다.


***


## 18. Swift 패키지 매니저 {.unnumbered}

파이썬에서는 모듈 의존성을 관리할 때 주로 `pip`, `requirements.txt`, `poetry` 같은 도구를 쓴다. Swift에서는 언어에 내장된 **Swift Package Manager (SwiftPM)** 가 표준이다.
...
```
그리고 이 정보를 `requirements.txt` 또는 `pyproject.toml` 등에 남긴다.

**Swift 의존성 관리:**
Swift에서는 프로젝트의 `Package.swift` 파일(매니페스트 파일)에 코드로 의존성을 관리한다. 이 파일 자체가 Swift 코드이다.
...
`swift build` 명령을 실행하면 이 매니페스트를 참고해 깃허브 저장소에서 라이브러리를 가져와 빌드 캐시에 저장한 뒤 애플리케이션을 컴파일한다. 파이썬의 `pip freeze`나 잠금 파일(Lockfile) 개념은 `Package.resolved`에 똑같이 반영된다.

가장 중요한 차이는 SwiftPM이 C, C++, Objective-C 코드의 직접적인 컴파일까지 지원해 네이티브 패키지 배포를 한 곳에서 해결한다는 점이다.


***


## 19. 명령줄 도구 만들기 {.unnumbered}

파이썬은 간단한 파일 입출력이나 텍스트 파싱을 하는 명령줄 도구(CLI)를 만드는 데 매우 인기 있는 언어이다. Swift는 정적 타이핑과 빠른 실행 속도 덕분에 명령줄 유틸리티를 만드는 새로운 대안으로 자리 잡고 있다.
...
SwiftPM에서 `swift-argument-parser` 패키지를 가져와 사용할 수 있다. 타입 안정성 덕분에 런타임에 인자 형식이 맞지 않으면 애플리케이션 프레임워크가 바로 사용법을 안내한다.
...
Greet.main()
```
Swift의 방식은 구조체의 프로퍼티(`name`, `count`)를 기반으로 코드가 스스로 문서가 되는(Self-Documenting) 아주 선언적인 방식이다. 빌드 결과물이 단일 바이너리(Single Binary)이므로 복사해서 배포하기도 수월한다.


***


## 20. 테스트 주도 개발 {.unnumbered}

파이썬 개발자들은 `unittest`, `pytest` 같은 도구로 강력한 테스트 환경을 경험해 왔다. Swift는 애플 생태계에서 전통적으로 XCTest를 써 왔지만 오픈 소스 **Swift Testing** 패키지가 공개되면서 테스트 작성법이 파이썬의 `pytest`처럼 한결 간결해졌다.
...
클래스 기반의 이러한 접근은 각 파일마다 중복되는 틀(Boilerplate)이 있었다.
...
Swift 6.0 및 Swift Testing 매크로를 이용하면 파이썬의 pytest처럼 함수형이고 간결한 테스트 블록을 만들 수 있다.
...
파이썬의 인자화된 테스트(`@pytest.mark.parametrize`)는 Swift의 매크로 기반 시스템(`@Test(arguments:)`)으로 구현되어 쓰는 느낌이 거의 같다.


***


## 21. Python과 Swift 연결하기 {.unnumbered}

머신 러닝과 스크립팅 생태계의 대명사인 파이썬을 빠르고 안전한 **Swift 안에서** 그대로 쓸 수 있다는 점이 가장 흥미롭다.

### PythonKit 소개

`PythonKit`은 Swift에서 파이썬 인터프리터의 API를 감싸서 파이썬 패키지를 Swift 환경으로 불러오고 마치 파이썬처럼 문법을 쓸 수 있게 해주는 마법 같은 패키지이다.

### Swift에서 Python Pandas 사용하기

만약 Swift로 데이터 분석 파이프라인이나 macOS 앱을 작성하면서도, 이미 구성된 파이썬 생태계(`numpy`, `pandas`, `matplotlib`)를 사용하고 싶다면 이 방식대로 구현하면 된다.
...
단, 이 코드가 작동하기 위해서는 실행 스크립트나 앱이 파이썬 인터프리터 동적 라이브러리(`libpython...dylib`, macOS 기준)에 접근할 수 있게 환경 변수(`PYTHON_LIBRARY`) 등을 구성해야 할 수도 있다.

### 결론

Swift 개발 중에 파이썬 생태계를 다뤄야 하는 상황이 와도 걱정할 필요는 없다. PythonKit이 두 언어의 장점을 한데 묶어 주는 다리가 되어 준다.
...
이 섹션에는 Swift 핵심 문법과 개념을 실습해 볼 수 있는 전체 예제 코드가 들어 있다.
...
이 섹션에는 배운 개념을 직접 코딩해 볼 수 있는 연습 문제 코드가 들어 있다.
...
## 부록 3: Swift 연습 문제 풀이 및 정답

이 섹션에는 연습 문제 해답과 자세한 설명이 달린 코드가 들어 있다.
